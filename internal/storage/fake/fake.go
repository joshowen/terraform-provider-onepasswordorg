...
id := sa.Name
if _, ok := r.serviceAccountsByID[id]; ok {
return nil, fmt.Errorf("service account already exists")
}

sa.ID = id
if sa.Token == "" {
sa.Token = "ops_fake_token_" + sa.Name
}
r.serviceAccountsByID[sa.ID] = sa

if err := r.dumpStorage(); err != nil {
return nil, err
}
...
sa, ok := r.serviceAccountsByID[id]
if !ok {
...
for _, sa := range r.serviceAccountsByID {
if sa.Name == name {
return &sa, nil
}
}
...
if _, ok := r.serviceAccountsByID[id]; !ok {
return fmt.Errorf("service account doesn't exist")
}

delete(r.serviceAccountsByID, id)

return r.dumpStorage()
}
